...
class base {
    ...
    public function get_userinfo($username) {
        ...
        if (auth_oidc_is_local_365_installed()) {
            ...
            if (!$userfromadditionaltenant) {
                if (\local_o365\feature\usersync\main::fieldmap_require_graph_api_call($eventtype)) {
                    ...
                    $firstname = $idtoken->claim('PrimerNombre');
                    if (!empty($firstname)) {
                        $userdata['givenName'] = $firstname;
                    }

                    //$lastname = $idtoken->claim('family_name');
                    $lastname = $idtoken->claim('PrimerApellido');
                    if (!empty($lastname)) {
                        $userdata['surname'] = $lastname;
                    }

                    $telefono = $idtoken->claim('Telefono');
                    if (!empty($telefono)) {
                        $userdata['mobilePhone'] = $telefono;
                    }
        ...
        if ($fieldmappingfromtoken) {
            ...
            $upn = $idtoken->claim('Identificacion');
            if (!empty($upn)) {
                $userdata['userPrincipalName'] = $upn;
            ...
            $direccion = $idtoken->claim('Direccion');
                    if (!empty($direccion)) {
                        $userdata['streetAddress'] = $direccion;
                    }
            
            $direccionJson = $idtoken->claim('DireccionJSON');
            $decoded_json = json_decode($direccionJson, false);
            $ciudad = $decoded_json->codigo_pais." -- ".$decoded_json->direccion;
                    if (!empty($ciudad)) {
                        $userdata['city'] = $ciudad;
                    }
            
            $pais = $decoded_json->codigo_pais;
                    if (!empty($pais)) {
                        $userdata['country'] = $pais;
                    }
